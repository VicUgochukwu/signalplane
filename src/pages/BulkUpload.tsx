...
import { Upload, X, CheckCircle2, AlertCircle, FileText, Loader2, ArrowLeft } from 'lucide-react';
...
export default function BulkUpload() {
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      formData.append('source_name', sourceName || 'Bulk Upload');
      ...
      <main className="flex-1 container max-w-3xl mx-auto px-4 py-8">
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            <Link
              to="/control-plane/submit"
              className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors"
            >
              <ArrowLeft className="w-3.5 h-3.5" />
              Submit a single signal
            </Link>
          </div>
          ...
          {!uploadResult && (
            <Card className="bg-card border-border">
              <CardHeader>
                ...
                <CardDescription>
                  Import competitive signals, objections, or feedback data from external sources.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      id="source-name"
                      placeholder="e.g., Gong Export, Zendesk Tickets"
                      value={sourceName}
          ...
          {/* Upload Result */}
          {uploadResult && (
            <Card className={`border ${uploadResult.success ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-destructive/10 border-destructive/30'}`}>
              <CardContent className="p-8 text-center">
                {uploadResult.success ? (
                  <>
                    <CheckCircle2 className="h-12 w-12 text-emerald-400 mx-auto mb-4" />
                    <h3 className="text-xl font-semibold text-foreground mb-2">
                      Successfully imported {uploadResult.validRows} signals
                    </h3>
                    <p className="text-muted-foreground mb-4">
                      Total: {uploadResult.totalRows} | Valid: {uploadResult.validRows} | Skipped: {uploadResult.skippedRows}
                    </p>
                    <p className="text-sm text-muted-foreground mb-6">
                      Signals will be processed and included in your next weekly packet.
                    </p>
                  </>
                ) : (
                  <>
